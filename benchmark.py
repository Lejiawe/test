...
def load_configured_run(path: Path) -> ConfiguredRun:
    ...
    api_key = product.get("api_key", "").strip()
    api_key_env = product.get("api_key_env", "").strip()
    if not api_key and api_key_env:
        api_key = os.environ.get(api_key_env, "").strip()
    if not api_key or api_key.upper().startswith(("REPLACE_", "YOUR_")):
        source = f" 或环境变量 {api_key_env}" if api_key_env else ""
        raise RuntimeError(f"[{profile_name}] 缺少有效的 api_key{source}")
    ...
    if not all(
        ...
        raise RuntimeError(f"[{profile_name}] extra_metadata_json 的键和值必须是字符串")
...
def summarize(
    ...
    groups: dict[tuple[str, str, str, int, float, int, int], list[TrialResult]] = {}
    for row in results:
        groups.setdefault(
            (
                row.provider,
...
def refresh_global_matrix(history_path: Path, matrix_path: Path) -> None:
    ...
    matrix: dict[tuple[str, str, str, str, str, str], dict[str, str]] = {}
    for row in history_rows:
    ...
    sorted_keys = sorted(
        ...
        key=lambda key: (
            key[0],
            key[3],
            duration_sort_value(key[4]),
            metric_order.get(key[5], len(metric_order)),
            key[1],
        ),
    )
    for (
        ...
        values = matrix[
            (test_name, provider, product_name, storage, duration_seconds, metric)
        ]
        matrix_rows.append(
            {
                "test_name": test_name,
