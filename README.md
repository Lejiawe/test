...
本项目是在 Windows 本地运行的压测客户端，通过 E2B Python SDK 调用阿里云或
字节云沙箱。它不会在本机部署云沙箱服务端。
...
3. 在对应产品段的 `api_key` 中填写该产品的新 API Key；
4. 检查该产品的 `api_url`、`domain`、`template` 和 `sandbox_timeout`。
...
`benchmark.template.ini` 已包含阿里云和字节的地址、域名及模板示例。真实
API Key只能填写在本地 `benchmark.ini` 或对应环境变量中，不能提交到GitHub。
...
`test_name + provider + product_name + storage + duration_seconds + metric` 唯一确定，各档位分别
放在 `1tps`、`10tps`、`50tps`、`100tps`、`200tps` 等列中。例如：

```text
test_name,provider,product_name,storage,duration_seconds,metric,unit,1tps,10tps,50tps
启动并发速度,aliyun,阿里云,none,60,api_latency_p50_ms,ms,420.1,510.2,890.4
启动并发速度,vol,字节,none,60,api_latency_p50_ms,ms,380.3,470.5,760.8
```
